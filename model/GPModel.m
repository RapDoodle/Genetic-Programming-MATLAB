    ...
    methods
        function model = GPModel(constructor)
            ...
            if ~isa(constructor(), 'GPMember')
                error('The passed in type does not implement the GPMember interface');
            end
            model.constructor = constructor;
        end
        
        function populate(model, populationSize)
            % Populate the GP model with populationSize of individuals
        ...
        function set.options(model, options)
            % Set the options for optimizing the GP model.
            % Arguments:
            %   
