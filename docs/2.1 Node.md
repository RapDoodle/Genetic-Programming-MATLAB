...
# EnvVariable
`EnvVariable` serves as a placeholder to inform the program to grab the corresponding variable from the context object provided. For example, 
```matlab
EnvVariable("Var1", 'bounded', 'double', min(data{:, 'Var1'}), max(data{:, 'Var1'}))
```
...
| EnvVariable | EnvVariable.\<VariableName\> |
...
classdef MyIfThenElse < IfThenElse
    ...
    methods
        ...
        function grow(node, maxHeight)
            ...
            node.ifNode = Equal();
            node.ifNode.lhs = EnvVariable("ValidPurchase", 'enumerated', 'int', [0, 1]);
...
```
Then, add `MyIfThenElse` and the `EnvVariable` for `ValidPurchase` to the template
```matlab
MyIfThenElse()
...
EnvVariable("SpecialVar1", 'bounded', 'double', min(data{:, 'SpecialVar1'}), max(data{:, 'Bid'}), {'valid'})
```
